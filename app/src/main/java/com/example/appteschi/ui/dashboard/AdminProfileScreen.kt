...
fun AdminProfileScreen(navController: NavController, matricula: String) {
    ...
    var observaciones by remember { mutableStateOf<List<ObservacionReglamento>>(emptyList()) }
    var cargandoObservaciones by remember { mutableStateOf(false) }
    var errorObservaciones by remember { mutableStateOf<String?>(null) }
    var mostrarNuevaObservacion by remember { mutableStateOf(false) }
    var nuevoMotivo by remember { mutableStateOf("") }
    var guardandoObservacion by remember { mutableStateOf(false) }
    var resolviendoId by remember { mutableStateOf<Int?>(null) }
    ...
    LaunchedEffect(matricula, recarga) {
        ...
        ReinscripcionAdminService.observaciones(matricula).fold(
            onSuccess = { observaciones = it; errorObservaciones = null },
            onFailure = { errorObservaciones = it.message ?: "No se pudieron cargar las observaciones de reglamento." }
        )
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (error != null) {
                item { Text(error!!, color = ErrorRed, fontSize = 13.sp) }
            }
            item {
                Card(
            ...
            if (perfil != null) {
                ...
                item {
                    ObservacionesCard(
                        ...
                        onAgregar = { mostrarNuevaObservacion = true },
                        onResolver = { id, autorizar ->
                            resolviendoId = id
                            scope.launch {
                                ReinscripcionAdminService.resolverObservacion(id, autorizar, null).fold(
                                    onSuccess = { recarga++ },
                                    onFailure = { e -> errorObservaciones = e.message ?: "No se pudo resolver la observación." }
                                )
                                resolviendoId = null
                            }
                        }
        ...
        AlertDialog(
            ...
            confirmButton = {
                Button(
                    ...
                    colors = ButtonDefaults.buttonColors(containerColor = if (activo) ErrorRed else GreenPrimary)
    ...
    if (mostrarNuevaObservacion) {
        AlertDialog(
            onDismissRequest = { if (!guardandoObservacion) { mostrarNuevaObservacion = false; nuevoMotivo = "" } },
            title = { Text("Registrar observación de reglamento") },
            text = {
                Column {
                    Text(
                        "El alumno quedará bloqueado para reinscribirse hasta que se resuelva esta observación.",
            ...
            confirmButton = {
                Button(
                    onClick = {
                        ...
                        scope.launch {
                            ReinscripcionAdminService.crearObservacion(matricula, nuevoMotivo.trim()).fold(
                                onSuccess = { mostrarNuevaObservacion = false; nuevoMotivo = ""; recarga++ },
                                onFailure = { e -> errorObservaciones = e.message ?: "No se pudo registrar la observación." }
                            )
                            guardandoObservacion = false
                        }
                    },
                    enabled = !guardandoObservacion && nuevoMotivo.isNotBlank(),
                    colors = ButtonDefaults.buttonColors(containerColor = ErrorRed)
                ) { Text(if (guardandoObservacion) "Guardando…" else "Registrar") }
            },
            dismissButton = {
                OutlinedButton(
                    onClick = { mostrarNuevaObservacion = false; nuevoMotivo = "" },
                    enabled = !guardandoObservacion
                ) { Text("Cancelar") }
            }
        )
    ...
    Card(colors = CardDefaults.cardColors(containerColor = Color.White), shape = RoundedCornerShape(16.dp), elevation = CardDefaults.cardElevation(defaultElevation = 2.dp)) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            when {
                ...
                error != null -> Text(error, color = ErrorRed, fontSize = 12.sp)
...
private fun HistorialMateriaRow(materia: MateriaCalificacion) {
    val (colorFondo, colorTexto) = when (materia.estatusCodigo) {
        "AP" -> SuccessContainer to OnSuccessContainer
        "NA" -> ErrorContainer to OnErrorContainer
        else -> WarningContainer to OnWarningContainer
    }
    Row(
...
/**
 * Observaciones de reglamento (Fase 2 de Reinscripción): mientras una siga
 * PENDIENTE, el alumno queda bloqueado en /api/reinscripcion/estatus hasta
 * que el director la autorice o la rechace — ver ReinscripcionViewModel.
 */
@Composable
private fun ObservacionesCard(
    observaciones: List<ObservacionReglamento>,
    cargando: Boolean,
    error: String?,
    resolviendoId: Int?,
    onAgregar: () -> Unit,
    onResolver: (id: Int, autorizar: Boolean) -> Unit
) {
    Card(colors = CardDefaults.cardColors(containerColor = Color.White), shape = RoundedCornerShape(16.dp), elevation = CardDefaults.cardElevation(defaultElevation = 2.dp)) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Icon(Icons.Default.Gavel, contentDescription = null, tint = GreenPrimary)
                Text("Observaciones de reglamento", color = GreenPrimary, fontWeight = FontWeight.Bold, fontSize = 18.sp)
            }
            Text(
                "Mientras una observación siga pendiente, el alumno no puede reinscribirse hasta que la autorices o la rechaces.",
                color = TextMuted, fontSize = 12.sp
            )
            if (error != null) Text(error, color = ErrorRed, fontSize = 12.sp)
            if (cargando) {
                ...
                Text("Sin observaciones registradas.", color = TextMuted, fontSize = 12.sp)
            } else {
                observaciones.forEach { obs ->
                    ObservacionRow(obs, resolviendo = resolviendoId == obs.id, onResolver = onResolver)
                }
            }
            OutlinedButton(onClick = onAgregar) { Text("Registrar observación") }
        }
    }
}

@Composable
private fun ObservacionRow(obs: ObservacionReglamento, resolviendo: Boolean, onResolver: (Int, Boolean) -> Unit) {
    val (fondo, color) = when (obs.estado) {
        "PENDIENTE" -> WarningContainer to OnWarningContainer
        "AUTORIZADA" -> SuccessContainer to OnSuccessContainer
        else -> ErrorContainer to OnErrorContainer
    }
    Card(colors = CardDefaults.cardColors(containerColor = fondo), shape = RoundedCornerShape(16.dp)) {
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text(obs.estado, color = color, fontWeight = FontWeight.Bold, fontSize = 12.sp)
                Text(obs.fechaRegistro, color = TextMuted, fontSize = 10.sp)
            }
            Text(obs.motivo, color = LoginInk, fontSize = 13.sp)
            Text("Registrada por: ${obs.registradaPor}", color = TextMuted, fontSize = 11.sp)
            if (obs.resueltaPor != null) {
                Text("Resuelta por: ${obs.resueltaPor}", color = TextMuted, fontSize = 11.sp)
            }
            if (obs.estado == "PENDIENTE") {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        onClick = { onResolver(obs.id, true) },
                        enabled = !resolviendo,
                        colors = ButtonDefaults.buttonColors(containerColor = GreenPrimary)
                    ) { Text(if (resolviendo) "…" else "Autorizar") }
                    OutlinedButton(onClick = { onResolver(obs.id, false) }, enabled = !resolviendo) {
                        Text("Rechazar")
                    }
                }
            }
