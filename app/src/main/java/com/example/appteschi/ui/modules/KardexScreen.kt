    ...
    Card(colors = CardDefaults.cardColors(containerColor = Color.White)) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ContadorChip("Aprobadas", uiState.materiasAprobadas, Color(0xFFE8F5E9), GreenPrimary, Modifier.weight(1f))
                ContadorChip("Por cursar", uiState.materiasPorAprobar, Color(0xFFF0F0F0), Color.Gray, Modifier.weight(1f))
                ContadorChip("No aprobadas", uiState.materiasReprobadas, Color(0xFFFFEBEE), Color(0xFFB3261E), Modifier.weight(1f))
            }
        }
    }
}
...
private fun MateriaRow(materia: MateriaHistorial) {
    val (colorFondo, colorTexto) = when (materia.estatus) {
        EstatusMateria.APROBADA -> Color(0xFFE8F5E9) to GreenPrimary
        EstatusMateria.POR_CURSAR -> Color(0xFFF0F0F0) to Color.Gray
        EstatusMateria.NO_APROBADA -> Color(0xFFFFEBEE) to Color(0xFFB3261E)
    }
    Card(colors = CardDefaults.cardColors(containerColor = Color.White)) {
        Row(
