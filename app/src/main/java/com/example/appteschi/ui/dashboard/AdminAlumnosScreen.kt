...
fun AdminAlumnosScreen(navController: NavController) {
    ...
    LaunchedEffect(recarga) {
        ...
        resultado.onSuccess { alumnos = it; listError = null }
            .onFailure { listError = "No se pudo cargar el directorio desde la base de datos." }
        if (carreras.isEmpty()) {
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (listError != null) {
                item { Text(listError!!, color = Color(0xFFB3261E), fontSize = 12.sp) }
            }
            if (cargando) {
                item {
        ...
        Column(modifier = Modifier.padding(14.dp)) {
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                ...
                Text(
                    ...
                    color = if (user.active) GreenPrimary else Color(0xFFB3261E),
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (UserSession.rol == "SUPERADMIN") {
                    Button(onClick = onEliminar, colors = ButtonDefaults.buttonColors(containerColor = Color(0xFFB3261E))) {
                        Text("Eliminar")
                    }
                }
            }
