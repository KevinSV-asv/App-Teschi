    ...
    Scaffold(
        ...
        if (error != null || datos == null) {
            Column(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
                Text(error ?: "Sin datos.", color = Color(0xFFB3261E))
            }
            return@Scaffold
        }
...
private fun ResumenActivos(d: EstadisticasResumen) {
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
        ...
        Kpi("Inactivos", d.inactivos.toString(), Color(0xFFB3261E), Modifier.weight(1f))
        Kpi("Sin semestre", d.sinSemestre.toString(), Color(0xFFB37B00), Modifier.weight(1f))
    }
}
